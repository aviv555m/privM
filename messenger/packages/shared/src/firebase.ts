...
export function getFirebaseClients() {
  if (!appInstance || !dbInstance || !authInstance || !storageInstance) {
    throw new Error('Firebase not initialized. Call initFirebase first.');
  }

  return { app: appInstance, db: dbInstance, auth: authInstance, storage: storageInstance };
}
...
async function uploadBlob(path: string, payload: Blob | ArrayBuffer) {
  const { storage } = getFirebaseClients();
  const fileRef = ref(storage, path);
  await uploadBytes(fileRef, payload instanceof Blob ? payload : new Blob([new Uint8Array(payload)]));
  return getDownloadURL(fileRef);
}

export async function uploadProfilePhoto(uid: string, payload: Blob | ArrayBuffer) {
  return uploadBlob(`users/${uid}/profile-${Date.now()}.jpg`, payload);
}

export async function uploadProfileBanner(uid: string, payload: Blob | ArrayBuffer) {
  return uploadBlob(`users/${uid}/banner-${Date.now()}.jpg`, payload);
}
...
export async function reserveUsername(uid: string, username?: string) {
  ...
  await setDoc(usernameRef, { uid, usernameLower: normalized, updatedAt: serverTimestamp() }, { merge: true });
  return normalized;
}
...
export async function upsertUserProfile(user: {
  ...
  const usernameLower = await reserveUsername(user.uid, username);
...
export async function editMessage(chatId: string, messageId: string, editorId: string, text: string) {
  ...
  if (message.deletedFor?.includes(editorId)) throw new Error('Cannot edit a hidden message');
...
export async function deleteMessage(chatId: string, messageId: string, editorId: string, scope: 'me' | 'everyone' = 'everyone') {
  const { db } = getFirebaseClients();
  const messageRef = doc(db, 'chats', chatId, 'messages', messageId);
  const snapshot = await getDoc(messageRef);
  if (!snapshot.exists()) throw new Error('Message not found');
  ...
  const message = snapshot.data() as Message;
  if (scope === 'everyone') {
    if (message.senderId !== editorId) throw new Error('Only sender can delete for everyone');
    await deleteDoc(messageRef);
    return;
  }

  await updateDoc(messageRef, {
    deletedFor: arrayUnion(editorId)
  });
}
...
export async function markMessagesSeen(chatId: string, uid: string) {
  ...
  messagesSnap.docs.forEach((entry) => {
    ...
    if (message.deletedFor?.includes(uid)) return;
...
export async function updateUserProfile(
  ...
  const usernameLower = await reserveUsername(uid, username);
