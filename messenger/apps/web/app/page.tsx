...
export default function HomePage() {
  ...
  const visibleMessages = useMemo(
    () => messages.filter((item) => !currentUser || !(item.deletedFor ?? []).includes(currentUser.uid)),
  ...
  async function handleDelete(messageId: string, scope: 'me' | 'everyone') {
    if (!currentUser || !selectedChatId) return;
    await deleteMessage(selectedChatId, messageId, currentUser.uid, scope);
      ...
      <section className="app-layout">
        <aside className="sidebar">
          ...
          <div className="search-box">
            <p className="section-title">New conversation</p>
            ...
            {selectedUsersForGroup.length > 0 ? (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
                {selectedUsersForGroup.map((uid) => (
                  <span key={uid} className="badge">{profilesById[uid]?.displayName ?? uid}</span>
                ))}
              </div>
            ) : null}
            <input className="input" value={groupName} onChange={(e) => setGroupName(e.target.value)} placeholder="Step 2: Group name" style={{ marginTop: 8 }} />
            ...
            <button className="primary-btn" style={{ marginTop: 8 }} onClick={handleCreateGroup}>Create WhatsApp-style group</button>
          </div>
                ...
                <article key={message.id} className={`bubble-wrap ${mine ? 'mine' : 'other'}`}>
                  <div className={bubbleClass}>
                    {editingMessageId === message.id ? (
                      ...
                      <div style={{ opacity: 0.7, fontStyle: 'italic' }}>This message was deleted.</div>
                    ) : (
                  ...
                  <div className="bubble-meta">
                    ...
                    {editingMessageId !== message.id ? <button className="meta-btn" onClick={() => setActiveMessageMenuId((v) => v === message.id ? null : message.id)}>▾</button> : null}
                    {activeMessageMenuId === message.id ? (
                      <div className="message-menu">
                        {canEdit ? <button className="meta-btn" onClick={() => { setEditingMessageId(message.id); setEditingText(message.text); setActiveMessageMenuId(null); }}>Edit</button> : null}
                        <button className="meta-btn" onClick={() => handleDelete(message.id, 'me')}>Delete for me</button>
                        {mine ? <button className="meta-btn" onClick={() => handleDelete(message.id, 'everyone')}>Delete for everyone</button> : null}
                      </div>
                    ) : null}
                  </div>
                </article>
